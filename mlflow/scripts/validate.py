...
def parse_args():
    ...
    parser.add_argument("--from-blob",type = bool, default=True,
                        help="Load model directly from blob storage instead of MLflow")
...
def ensure_no_active_run():
    ...
    """
    try:
        # Loop until mlflow.active_run() returns None
...
def validate_model(
    ...
    try:
        ...
        mlflow.log_params({
            ...
            "loading_from_blob": from_blob
        })

        # 7. Load model from registry or blob storage
        try:
            ...
            logger.error(f"Failed to load model from primary source: {load_err}")
        ...
        for k, v in metrics.items():
            ...
            elif k == 'confusion_matrix_fig' and isinstance(v, plt.Figure):
                # Save confusion matrix figure if available
                fig_path = os.path.join(mlflow.get_artifact_uri(), "confusion_matrix.png")
                v.savefig(fig_path)
        ...
        # 11. Attach tags to model version
        ...
        try:
            for tag, val in tags_to_set.items():
                registry.client.set_model_version_tag(model_name, version, tag, val)
            logger.info(f"Added validation tags to model version")
        except Exception as tag_err:
            logger.warning(f"Failed to set model version tags: {tag_err}")

        # 12. Promote to Production if passed and requested
        if passed and promote_to_prod:
            try:
                ...
                logger.error(f"Failed to promote model to Production: {promote_err}")
                mlflow.log_metric('promotion_failed', 1)
        ...
        # 13. Ensure run is ended
        mlflow.end_run()

# ==== Entry point ====
if __name__ == '__main__':
    args = parse_args()
    # Ensure clean state before validating
    ensure_no_active_run()
    ...
    try:
        ...
        sys.exit(2)  # Exit with code 2 for errors
